...
(pas de SSG, pas de templating serveur — `middleware.js` gère uniquement l'A/B test
`index.html` / `index-b.html`). Le footer (`<footer class="sfooter">`) est dupliqué
tel quel dans 7 fichiers : `index.html`, `index-b.html`, `b2b.html`, `b2c.html`,
...
2. **Footer** (7 fichiers existants + 10 nouvelles pages villes) :
...
- Modification du système d'A/B test (`middleware.js`) — les pages villes ne sont
  pas concernées par le test `index.html`/`index-b.html`.
...
/index.html, index-b.html,
 ...
 resultats.html, blog.html       (modifiés — footer + colonne Villes + lien plan du site)
```
