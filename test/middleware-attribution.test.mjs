...
test('un slug connu dans ?ref est pose en cookie ms_ref', () => {
  const response = call(`https://cabinetms.fr/b2b.html?ref=${SLUG}`);
...
test('les parametres utm survivent au nettoyage du slug', () => {
  const response = call(
    `https://cabinetms.fr/b2b.html?utm_source=commercial&ref=${SLUG}&utm_campaign=${SLUG}`
  );
...
test('un slug inconnu est ignore plutot que stocke', () => {
  const response = call('https://cabinetms.fr/b2b.html?ref=slug-invente');
  assert.equal(
    refCookie(response),
    null,
    'sans whitelist, n_importe quel visiteur peut s_attribuer un dossier'
  );
  assert.equal(response.headers.get('location'), 'https://cabinetms.fr/b2b.html');
});

test('first-touch : un ms_ref existant n_est pas ecrase', () => {
  const response = call(`https://cabinetms.fr/b2b.html?ref=${OTHER_SLUG}`, {
    cookie: `ms_ref=${SLUG}`
  });
  assert.equal(
    refCookie(response),
...
test('un lien court inconnu redirige sans attribuer', () => {
  const response = call('https://cabinetms.fr/c/slug-invente');
  assert.equal(response.status, 302);
  assert.equal(response.headers.get('location'), 'https://cabinetms.fr/b2b.html');
  assert.equal(refCookie(response), null);
});
...
test('la page reste servie quand aucun ref n_est present', () => {
  const response = call('https://cabinetms.fr/b2b.html');
  assert.equal(refCookie(response), null);
...
test('un code de campagne connu dans ?camp est pose en cookie ms_camp', () => {
  const response = call(`https://cabinetms.fr/b2b.html?camp=${CAMP}`);
...
test('un code de campagne inconnu est ignore plutot que stocke', () => {
  const response = call('https://cabinetms.fr/b2b.html?camp=campagne-inventee');
  assert.equal(campCookie(response), null);
  assert.equal(response.headers.get('location'), 'https://cabinetms.fr/b2b.html');
});

test('dernier-touch : un nouveau camp ecrase un ms_camp existant', () => {
  const response = call(`https://cabinetms.fr/b2b.html?camp=${OTHER_CAMP}`, {
    cookie: `ms_camp=${CAMP}`
  });
  assert.equal(
    campCookie(response),
...
test('ref et camp cohabitent sur le meme lien', () => {
  const response = call(`https://cabinetms.fr/b2b.html?ref=${SLUG}&camp=${CAMP}`);
  assert.equal(refCookie(response), SLUG);
  assert.equal(campCookie(response), CAMP);
  const location = new URL(response.headers.get('location'));
  assert.equal(location.searchParams.get('ref'), null);
