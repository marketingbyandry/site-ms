...
4. Push the branch and open a PR with `gh pr create --fill` (in web sessions without `gh`, use the GitHub MCP tool `create_pull_request`).
5. Once checks pass, merge with `gh pr merge --squash` (or the GitHub MCP tool `merge_pull_request`, squash method) and report the PR number + merge commit SHA.
